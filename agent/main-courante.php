...
try {
    // Utilisation de utilisateur_id au lieu de agent_id
    $query = "SELECT mc.*, c.nom AS categorie_name, sc.nom AS sous_categorie_name 
              FROM main_courante mc
              JOIN categories c ON mc.categorie_id = c.id
              JOIN sous_categories sc ON mc.sous_categorie_id = sc.id
              WHERE mc.utilisateur_id = :utilisateur_id
              ORDER BY mc.date_creation DESC"; // Ajout du tri par date décroissante
    $stmt = $pdo->prepare($query);
    $stmt->execute(['utilisateur_id' => $_SESSION['user_id']]); // Passer utilisateur_id dans l'exécution
...
<link rel="stylesheet" href="./assets/css/theme.min.css">
      ...
      <title>Mes Main Courantes</title>
   </head>
...
<!-- fin de la barre de navigation -->
    ...
    <h3 class="text-white mb-1 font-normal">Mes Main Courantes</h3>
</div>
    ...
    <div class="container mx-auto p-6">
        ...
        <div class="flex justify-between items-center mb-6">
          <h3 id="main-title" class="text-3xl font-semibold text-gray-900 tracking-tight">Mes Main Courantes</h3>
            <a href="creat-main-courante.php" class="bg-indigo-600 text-white px-6 py-3 rounded-lg shadow-lg hover:bg-indigo-700 transition duration-300 transform hover:scale-105">Ajouter Main Courante</a>
        </div>

        <?php if (!empty($main_courantes)): ?>
        <div class="overflow-x-auto bg-white shadow-lg rounded-lg p-4">
            <table class="table-auto w-full text-sm text-left text-gray-800 sm:table-auto sm:w-full">
                <thead class="bg-indigo-700 text-white"> <!-- Arrière-plan coloré pour l'en-tête -->
                    <tr>
                        <th class="px-4 py-2 font-medium">Date</th>
                        <th class="px-4 py-2 font-medium">Commentaire</th>
                        <th class="px-4 py-2 font-medium">Catégorie</th>
                        <th class="px-4 py-2 font-medium">Sous-catégorie</th>
                        <th class="px-4 py-2 font-medium">Action</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($main_courantes as $main): ?>
                    <tr class="border-b hover:bg-indigo-50 transition duration-300">
                        <td class="px-4 py-2"><?php echo date('d/m/Y H:i', strtotime($main['date_creation'])); ?></td>
                        <td class="px-4 py-2"><?php echo htmlspecialchars($main['commentaire']); ?></td>
                        <td class="px-4 py-2"><?php echo htmlspecialchars($main['categorie_name']); ?></td>
                        <td class="px-4 py-2"><?php echo htmlspecialchars($main['sous_categorie_name']); ?></td>
                        <td class="px-4 py-2">
                            <a href="edit-main-courante.php?id=<?php echo $main['id']; ?>" class="text-indigo-600 hover:text-indigo-800 font-semibold transition duration-300 transform hover:scale-105">Modifier</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
