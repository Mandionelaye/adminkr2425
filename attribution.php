...
if (!isset($_SESSION['user_id']) || !isset($_SESSION['user_role']) || $_SESSION['user_role'] !== 'entreprise') {
    header("Location: login.php");
    exit();
}

// Liste des attributions
$query = "SELECT attribution.id_att, utilisateurs.prenom, utilisateurs.nom, utilisateurs.telephone, bj_site.name AS site_nom 
          FROM attribution
          JOIN utilisateurs ON attribution.id_agent = utilisateurs.id
          JOIN bj_site ON attribution.id_site = bj_site.id
          WHERE utilisateurs.role IN ('agent', 'controleur')";
...
<html lang="fr">
   <head>
      ...
      <title>Gérer les attributions</title>
   </head>
   <body>
     <main>
         <!-- start the project -->
         <!-- app layout -->
            <div id="app-layout" class="overflow-x-hidden flex">
                   ...
                   <div id="app-layout-content" class="min-h-screen w-full min-w-[100vw] md:min-w-0 ml-[15.625rem] [transition:margin_0.25s_ease-out]">
                        ...
                        <h3 class="text-white mb-1 font-normal">Gérer les Attributions</h3>
                        </div>
                        ...
                        <?php elseif (isset($_GET['error']) && $_GET['error'] == 'duplicate'): ?>
                            <div class="bg-red-200 text-red-800 p-4 mb-6 rounded-lg font-semibold">
                                Cet employé est déjà attribué à ce site.
                            </div>
                        <?php endif; ?>

                        <div class="card shadow-lg">
                            <div class="card-body">
                                <h5 class="text-2xl font-semibold text-gray-800 mb-6">Liste des Attributions</h5>

                                <!-- Bouton Ajouter qui redirige vers la page d’attribution -->
                                 
                                
                                 <div class="mb-4">
                                    <a href="attribuer-employes.php?id_site=<?= $_GET['id_site']; ?>" 
                                ...
                                <div class="mb-4 flex justify-end">
                                    <a href="manage-site.php" 
                                    class="px-3 py-2 bg-blue-600 text-white rounded hover:bg-green-700 flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                            <path fill-rule="evenodd" d="M12.293 5.293a1 1 0 011.414 0l5 5a1 1 0 010 1.414l-5 5a1 1 0 11-1.414-1.414L15.586 11H3a1 1 0 110-2h12.586l-3.293-3.293a1 1 0 010-1.414z" clip-rule="evenodd" />
                                        </svg>
                                    </a>
                                </div>
                                    ...
                                    <table class="min-w-full table-auto border-separate border-spacing-0 rounded-lg overflow-hidden">
                                        ...
                                        <tbody>
                                            <?php if (count($attributions) > 0): ?>
                                                <?php foreach ($attributions as $att): ?>
                                                    <tr class="hover:bg-gray-100 transition duration-200">
                                                        <td class="px-6 py-4 text-gray-800"><?= htmlspecialchars($att['prenom']); ?></td>
                                                        <td class="px-6 py-4 text-gray-800"><?= htmlspecialchars($att['nom']); ?></td>
                                                        <td class="px-6 py-4 text-gray-800"><?= htmlspecialchars($att['telephone']); ?></td>
                                                        <td class="px-6 py-4 text-gray-800"><?= htmlspecialchars($att['site_nom']); ?></td>
                                                        <td class="">
                                                        <div class="flex space-x-3 justify-start"> <!-- Augmentation de l'espace -->
                                                            <a href="supprimer-attribution.php?id=<?= $att['id_att']; ?>" 
                                            ...
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="4" class="px-4 py-2 text-center text-gray-600">Aucune attribution enregistrée.</td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
                                </table>
