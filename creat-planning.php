...
$stmt_age = $pdo->prepare("SELECT email FROM utilisateurs WHERE id = ?");
$stmt_age->execute([$agent_id]);
$agent = $stmt_age->fetch(PDO::FETCH_ASSOC);

$to = $agent['email'];  // L'adresse email du destinataire
$subject = 'Test de l\'envoi d\'un email';  // Le sujet de l'email
$message = 'Bonjour, ceci est un message de test envoyé depuis PHP.';  // Le contenu du message

           $headers = 'From: contactest@ashramsenegal.com' . "\r\n" .
           'Reply-To: contactest@ashramsenegal.com' . "\r\n" .
           'X-Mailer: PHP/' . phpversion();

// Configuration du serveur SMTP SendGrid
// ini_set("SMTP", "mail.ashramsenegal.com");
// ini_set("smtp_port", "465");
// ini_set("sendmail_from", "contactest@ashramsenegal.com");
...
if($agent){
   ...
   if (mail($to, $subject, $message, $headers)) {
      $valid_email = "L'email a été envoyé avec succès.";
   } else {
      $valid_email = "Une erreur est survenue lors de l'envoi de l'email. ".$to;
   }
}

        $_SESSION['message'] = "Planning créé avec succès ! ".$valid_email.' '.$agent_id;
...
// Récupérer la liste des agents pour l'entreprise
$stmt_agents = $pdo->prepare("SELECT id, nom, prenom, email FROM utilisateurs WHERE entreprise_id = ? AND role = 'agent'");
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                ...
                <?php foreach ($jours as $jour): ?>
                    <label class="flex items-center space-x-2">
                        <input type="checkbox" name="dates[]" value="<?php echo $jour; ?>" class="form-checkbox text-indigo-600" onchange="updateSchedule()">
...
function updateSchedule() {
    ...
    selectedDays.forEach(day => {
        ...
        dayField.innerHTML = `
            ...
            <label class="block text-gray-700 font-medium mb-2">Site</label>
            <input type="text" name="site[${day}]" class="w-full p-2 border border-gray-300 rounded" required>
        `;
